...
- A-06 больше не `TODO`: PR/push CI теперь запускает install/syntax sanity и `pytest`;
...
4. test matrix на Python 3.12 и 3.13
5. upload `pytest` JUnit artifacts
...
2. вернуть `ruff` и `mypy` как обязательный gate после погашения текущего static-analysis debt
3. coverage upload
4. `pip-audit`
